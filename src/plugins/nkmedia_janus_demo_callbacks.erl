...
plugin_syntax() ->
    nkpacket:register_protocol(janus, nkmedia_janus),
    nkpacket:register_protocol(janus_proxy, nkmedia_fs_janus_proxy_server),
    #{
        janus_listen => fun parse_listen/3,
        janus_web => fun parse_listen/3
    }.


plugin_listen(Config, #{id:=SrvId}) ->
    % janus_listen will be already parsed
    Listen1 = maps:get(janus_demos, Config, []),
    ListenPath1 = list_to_binary(code:priv_dir(nkmedia)),
    ListenPath2 = <<ListenPath1/binary, "/www/janus">>,
    JanusOpts2 = #{
        class => {nkmedia_sample_janus, SrvId},
        http_proto => {static, #{path=>ListenPath2, index_file=><<"index.html">>}}
    },
    Listen2 = [{Conns, maps:merge(ConnOpts, JanusOpts2)} 
                    || {Conns, ConnOpts} <- Listen1],
    Web1 = maps:get(janus_web, Config, []),
    Path1 = list_to_binary(code:priv_dir(nkmedia)),
    Path2 = <<Path1/binary, "/www/janus">>,
    Opts2 = #{
        class => {nkmedia_janus_vc, SrvId},
...
parse_listen(Key, Url, _Ctx) ->
    Schemes = case Key of
        janus_listem -> [janus, janus_proxy];
        janus_web -> [https]
    end,
