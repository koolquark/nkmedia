...
conn_init(NkPort) ->
    ...
    case nkmedia_kms_engine:get_all() of
        [{Name, KurentoPid}|_] ->
            case nkmedia_kms_proxy_client:start(KurentoPid) of
                {ok, ProxyPid} ->
        ...
        [] ->
            ?LLOG(error, "could not locate any Kurento server", [], State),
            {stop, no_kms_server}
    end.
...
conn_parse({text, Data}, _NkPort, #state{proxy=Pid}=State) ->
    ...
    nkmedia_kms_proxy_client:send(Pid, self(), Msg),
    {ok, State}.
