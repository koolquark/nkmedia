...
%% @doc 
-module(nkmedia_kms_proxy_server).
...
-type user_state() :: nkmedia_kms_proxy:state().
...
default_port(ws) -> 8001;
default_port(wss) -> 8002.
...
conn_parse({text, Data}, _NkPort, #state{proxy=Pid}=State) ->
    ...
    case handle(nkmedia_kms_proxy_in, [Msg], State) of
        {ok, Msg2, State2} ->
            ok = nkmedia_kms_proxy_client:send(Pid, self(), Msg2),
            {ok, State2};
        {stop, Reason, State2} ->
            {stop, Reason, State2}
    end.
...
conn_handle_call({send_reply, _Pid, Event}, From, NkPort, State) ->
    ...
    case handle(nkmedia_kms_proxy_out, [Event], State) of
        {ok, Event2, State2} ->
...
conn_handle_call(Msg, From, _NkPort, State) ->
    handle(nkmedia_kms_proxy_handle_call, [Msg, From], State).
...
conn_handle_cast(Msg, _NkPort, State) ->
    handle(nkmedia_kms_proxy_handle_cast, [Msg], State).
...
conn_handle_info(Msg, _NkPort, State) ->
    handle(nkmedia_kms_proxy_handle_info, [Msg], State).
...
conn_stop(Reason, _NkPort, State) ->
    catch handle(nkmedia_kms_proxy_terminate, [Reason], State).
...
%% @private
connect([], _State) ->
    {stop, no_kms_available};

connect([Name|Rest], State) ->
    case nkmedia_kms_proxy_client:start(Name) of
        {ok, ProxyPid} ->
            ?LLOG(info, "connected to Kurento server ~s", [Name], State),
