...
-export([nkmedia_kms_proxy_init/2, nkmedia_kms_proxy_find_kms/2,
         ...
         nkmedia_kms_proxy_handle_cast/2, nkmedia_kms_proxy_handle_info/2]).
...
plugin_syntax() ->
    ...
    nkpacket:register_protocol(kmss, nkmedia_kms_proxy_server),
    #{
...
plugin_listen(Config, #{id:=SrvId}) ->
    % kurento_proxy will be already parsed
    Listen = maps:get(kurento_proxy, Config, []),
...
-spec nkmedia_kms_proxy_init(nkpacket:nkport(), state()) ->
    {ok, state()}.

nkmedia_kms_proxy_init(_NkPort, State) ->
    {ok, State}.


%% @doc Called to select a KMS server
-spec nkmedia_kms_proxy_find_kms(nkmedia_service:id(), state()) ->
...
-spec nkmedia_kms_proxy_in(map(), state()) ->
    {ok, map(), state()} | {stop, term(), state()} | continue().

nkmedia_kms_proxy_in(Msg, State) ->
    {ok, Msg, State}.


%% @doc Called when a new msg is to be answered
-spec nkmedia_kms_proxy_out(map(), state()) ->
    {ok, map(), state()} | {stop, term(), state()} | continue().

nkmedia_kms_proxy_out(Msg, State) ->
    {ok, Msg, State}.


%% @doc Called when the connection is stopped
-spec nkmedia_kms_proxy_terminate(Reason::term(), state()) ->
    {ok, state()}.

nkmedia_kms_proxy_terminate(_Reason, State) ->
    {ok, State}.


%% @doc 
-spec nkmedia_kms_proxy_handle_call(Msg::term(), {pid(), term()}, state()) ->
    {ok, state()} | continue().

nkmedia_kms_proxy_handle_call(Msg, _From, State) ->
    lager:error("Module ~p received unexpected call: ~p", [?MODULE, Msg]),
    {ok, State}.


%% @doc 
-spec nkmedia_kms_proxy_handle_cast(Msg::term(), state()) ->
    {ok, state()}.

nkmedia_kms_proxy_handle_cast(Msg, State) ->
    lager:error("Module ~p received unexpected cast: ~p", [?MODULE, Msg]),
    {ok, State}.


%% @doc 
-spec nkmedia_kms_proxy_handle_info(Msg::term(), state()) ->
    {ok, State::map()}.

nkmedia_kms_proxy_handle_info(Msg, State) ->
...
parse_listen(kurento_proxy, Url, _Ctx) ->
    Opts = #{valid_schemes=>[kms, kmss], resolve_type=>listen},
