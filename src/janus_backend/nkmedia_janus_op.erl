...
-define(USING_TRICKLE, false).     % Change to false if Verto client has no trickle
...
handle_cast({candidate, Candidate}, State) ->
    ...
    Handle = Handle1,
    % case Candidate of
    %     #candidate{type=answer} when is_integer(Handle2) -> Handle2;
    %     _ -> Handle1
    % end,
