...
NkMEDIA also offers out of the box three signaling APIs (again as plugins): a full SIP implementation (based on [NkSIP](https://github.com/NetComposer/nksip), so it can be a flexible, massively scalable SIP client and server), a [Verto](http://evoluxbr.github.io/verto-docs/) server implementation (that can be used with any backend, not only Freeswitch) and its own signaling system, [NetComposer WS API](https://github.com/NetComposer/nkservice/blob/luerl/doc/api_intro.md). It also possible to add new signaling APIs, or use your own signalling solution out of NkMEDIA:
